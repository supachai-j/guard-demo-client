...
async def run_multi_provider(payload: MultiProviderRunRequest, db: Session = Depends(get_db)):
    ...
    Delegates to `_run_playbook_against_providers` so throttling +
    null-status warning surfacing stay consistent with `/matrix`.
    """
    ...
    result = await _run_playbook_against_providers(payload.playbook_id, payload.provider_ids, db)
    # The helper returns `{"error": "..."}` if the playbook doesn't exist
    # (since it's called by /matrix which collects per-playbook errors instead
    # of raising). For the single-playbook endpoint, surface as 404 like before.
